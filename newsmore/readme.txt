=== NewsMore ===
Contributors: afthemes
Requires at least: 4.0
Requires PHP: 5.3
Tested up to: 6.8
Stable tag: 1.2.48
License: GNU General Public License v2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html
Tags: news, blog, entertainment, one-column, two-columns, three-columns, four-columns, grid-layout, block-patterns, block-styles, left-sidebar, right-sidebar, custom-header, flexible-header, custom-background, custom-logo, custom-menu, custom-colors, featured-images, full-width-template, post-formats, rtl-language-support, footer-widgets, translation-ready, theme-options, threaded-comments, wide-blocks

NewsMore is a high-performance and versatile WordPress theme designed for news, blog, and magazine websites. Built on the lightweight MoreNews parent theme, it offers over 30 custom widgets, block patterns, and flexible layout options for dynamic content presentation. Fully compatible with Gutenberg and Elementor, it ensures easy customization, while its multilingual and RTL support, including demo content in languages like English, Chinese, Spanish, French, Japanese, and Arabic, makes it ideal for global publishers. With seamless WooCommerce integration, light/dark mode options, optimized SEO settings, and fast loading speeds, NewsMore provides everything needed for a professional, modern, and responsive site. Explore demos and documentation at https://afthemes.com/products/newsmore 

== Copyright ==

This theme, like WordPress, is licensed under the GPL.
Use it to make something cool, have fun, and share what you've learned with others.

NewsMore is based on Underscores https://underscores.me/, (C) 2012-2017 Automattic, Inc.
Underscores is distributed under the terms of the GNU GPL v2 or later.

NewsMore is a child theme of "MoreNews". MoreNews Theme is licensed under the GPL v2. Copyright 2025 AF themes
NewsMore is distributed under the terms of the GNU GPL v2 or later.

== Description ==
 NewsMore is a versatile and high-performance child theme built to elevate news, blog, and magazine websites with powerful features and modern design options. While maintaining the lightweight structure of its parent theme, MoreNews, it provides 30+ custom widgets, block patterns, flexible layout options, and unique banner styles for a dynamic and engaging reader experience. NewsMore includes specialized demo sites that are easily customizable with Gutenberg and Elementor, ensuring a quick and hassle-free setup. With built-in multilingual support and pre-imported demo content in languages like English, Chinese, Spanish, French, Japanese, and Arabic, this theme is perfect for global publishers. Seamless WooCommerce integration allows you to transform your site into an online store, and advanced options like light/dark mode, RTL support, and optimized SEO settings ensure maximum user visibility and accessibility. Backed by outstanding performance scores and fast loading speeds, NewsMore provides a professional platform tailored for impactful content delivery. Explore demos and documentation at https://afthemes.com/products/newsmore 

== Installation ==

1. In your WordPress dashboard, go to Appearance > Themes and click Add New.
2. Click on Upload Theme, then select the theme’s .zip file from your computer. Click Install Now.
3. After installation, click Activate to start using the theme.
4. To import demo content, go to Starter Sites under the NewsMore menu in your dashboard.
5. To import Gutenberg block patterns, go to Block Patterns under the NewsMore menu.
6. To import Elementor template kits, go to Elementor Kits under the NewsMore menu.

== Frequently Asked Questions ==

= Does this theme support any plugins? =

NewsMore includes support for the following plugins:

- **Jetpack**: Compatible with Infinite Scroll, Content Options, and Related Posts.
- **Gutenberg**: Supports a variety of block patterns for enhanced customization.
- **Elementor**: Compatible with template kits and custom widgets for advanced layouts.
- **WooCommerce**: Fully compatible with eCommerce functionality, including product grids and custom shop layouts.
- **Contact Form 7**: Easily create contact forms and integrate them into your pages.
- **WPML**: Ready for multilingual websites, making translation and localization seamless.
- **Yoast SEO**: Supports enhanced SEO features and breadcrumbs for improved visibility.

== Change-log ==
= 1.2.48 - Jun 24 2025 =
* Update - Footer Style improvement
* Fixed - Some styling glitch

= 1.2.47 - Jun 19 2025 =
* Update - Footer Style improvement
* Fixed - Some styling glitch

= 1.2.45 - May 23 2025 =
* Update - Footer Style improvement
* Fixed - Some styling glitch

= 1.2.44 - May 11 2025 =
* Update - Footer Style improvement
* Fixed - Some styling glitch

= 1.2.43 - Apr 22 2025 =
* Update - Block Style improvement
* Fixed - Some styling glitch

= 1.2.41 - Apr 03 2025 =
* Update - Header improvement
* Update - Block Style improvement
* Fixed - Some styling glitch

= 1.2.38 - Mar 04 2025 =
* Update - Elementor Style improvement
* Update - Block Style improvement
* Fixed - Some styling glitch

= 1.2.37 - Feb 28 2025 =
* New - Header Image Position Option
* Option - Customizing > Header Options > Header Image > Header Image Mode
* Update - Block Style improvement
* Fixed - Some styling glitch

= 1.2.35 - Jan 29 2025 =
* Update - Banner improvement
* Update - Style improvement

= 1.2.34 - Jan 24 2025 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.33 - Jan 17 2025 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.32 - Jan 14 2025 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.31 - Jan 10 2025 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.30 - Jan 08 2025 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.29 - Jan 04 2025 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.28 - Jan 02 2025 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.27 - Dec 31 2024 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.26 - Dec 30 2024 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.25 - Dec 29 2024 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.24 - Dec 28 2024 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.23 - Dec 27 2024 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.22 - Dec 26 2024 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.21 - Dec 23 2024 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.20 - Dec 20 2024 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.19 - Dec 17 2024 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.18 - Dec 15 2024 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.17 - Dec 13 2024 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.16 - Dec 09 2024 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.15 - Dec 07 2024 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.14 - Dec 06 2024 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.13 - Dec 04 2024 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.12 - Nov 30 2024 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.11 - Nov 29 2024 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.10 - Nov 27 2024 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.9 - Nov 26 2024 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.8 - Nov 23 2024 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.7 - Nov 21 2024 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.6 - Nov 20 2024 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.5 - Nov 17 2024 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.4 - Nov 16 2024 =
* Update - Style improvement
* Update - Block Style improvement

= 1.2.2 - Nov 07 2024 =
* Update - Featured Posts section title
* Option - Appearance > Customize > Frontpage Options > Featured Posts

= 1.2.0 - Nov 03 2024 =
* New - 3 Columns Posts List Section
* Option - Appearance > Customize > Frontpage Options Posts List
* Update - Responsive improvement
* Update - Style improvement 

= 1.0.1 - Oct 30 2024 =
* Customizer default value update

= 1.0.0 - Oct 24 2024 =
* Initial release


------------------------------------------------------------------------------
Image License
------------------------------------------------------------------------------
License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/99818

License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/1081782

License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/1047693

License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/658894

License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/1324628

License: CC0 1.0 Universal (CC0 1.0)
License URL: https://pxhere.com/en/license
Source: https://pxhere.com/en/photo/535638